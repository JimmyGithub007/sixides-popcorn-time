                            ...
                            <div className="flex gap-3 overflow-x-auto">
                                {
                                    casts?.map(value => (
                                        <div className="flex flex-col gap-1 pt-2 text-sm" key={value.id}>
                                            {   value.profile_path ? 
                                                <Poster type="media" width={138} height={175} className="rounded-md shadow-lg shadow-slate-800" poster_path={`${process.env.NEXT_PUBLIC_POSTER_API}w138_and_h175_face/${value.profile_path}`} /> :
                            ...
                            <div className="flex gap-3 overflow-x-auto overflow-y-hidden">
                                {
                                    images?.map((value, key) => (
                                        <div key={key} className="w-[533px] h-[300px]">
                                            <Poster type="media" width={533} height={300} className="rounded-md shadow-lg shadow-slate-800" poster_path={`${process.env.NEXT_PUBLIC_POSTER_API}w1066_and_h600_face/${value.file_path}`} />
                                        </div>
                                    ))
                                }
                            </div>
