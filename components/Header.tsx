...
const Header = () => {
    ...
    return (<><motion.div
        ...
        <div className="flex flex-row items-center gap-4">
            {/*<div className={`${scrollY > 0 ? "text-black shadow-lg rounded-full" : "text-white"} text-3xl cursor-pointer duration-300`}><FaUserCircle /></div>*/}
        ...
        <AnimatePresence>
            {openSearchBar &&
                ...
                >
                    <input onKeyDown={(e) => { if (e.key === "Enter" && !loading && keyword) router.push("/movie/p/1?keyword=" + keyword); }} value={keyword || ""} onChange={e => {
                        dispatch(setKeyword(e.target.value));
                    }} className="bg-black text-white h-12 text-right w-full focus:outline-none px-4" placeholder="Search for a movie name" />
                    <button onClick={() => {
                        if (!loading && keyword) router.push("/movie/p/1?keyword=" + keyword);
                    ...
                    <button onClick={() => { if (!loading) { dispatch(setKeyword("")); dispatch(setOpenSearchBar(false)); router.push("/movie/p/" + page); } }} className={`${loading && "cursor-not-allowed opacity-50"} text-white font-bold text-xl`}><IoClose /></button>
                </motion.div>
            }
        </AnimatePresence>
    </motion.div>
